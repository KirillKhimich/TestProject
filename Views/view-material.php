        <?php foreach ($viewMaterial as $key => $value) :?>
            ...
            <div class="row">
                    ...
                    <ul class="list-group mb-4">
                        <?php if (!empty($viewMaterialTags)){foreach ($viewMaterialTags as $key => $value):?>
                            <li class="list-group-item list-group-item-action d-flex justify-content-between">
                                <a href="tag-material?viewTagId=<?=$value['id']?>" class="me-3">
                                    <?=$value['tagsTitle']?>
                                </a>
                                <a href="#deleteModalToggle" data-bs-toggle="modal" role="button" class="text-decoration-none delete"  data-id="viewMaterialTags/<?=$value['id']?>">
                ...
                <div class="col-md-6">
                    ...
                    <ul class="list-group mb-4 listLinks">
                       <?php if(!empty($viewAllLinks)){foreach ($viewAllLinks as $key => $value) :?>
                        <li class="list-group-item list-group-item-action d-flex justify-content-between ">
                            <a href="<?=$value['link']?>" target="_blank" id="<?=$value['id']?>" data-id="<?=$value['title']?>" class="me-3">
                                <?=$value['title']?>
                            </a>
                            <span class="text-nowrap">
                            <a data-bs-toggle="modal" href="#exampleModalToggle1" data-id="<?=$value['id']?>" role="button" class="text-decoration-none me-2 editLink">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                     class="bi bi-pencil" viewBox="0 0 16 16">
                        <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                    </svg>
                            </a>
                        <a href="#deleteModalToggle" data-bs-toggle="modal" role="button" class="text-decoration-none delete" data-id="viewMaterialLinks/<?=$value['id']?>" >
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                 class="bi bi-trash" viewBox="0 0 16 16">
                                <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/>
                                <path fill-rule="evenodd"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="submit" name="addMaterialLink" id="addMaterialLink" class="btn btn-primary">Добавить</button>
            ...
            <div class="modal fade" id="exampleModalToggle1" aria-hidden="true" aria-labelledby="exampleModalToggleLabel"
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" name="updateLinksTitle" placeholder="Добавьте подпись"
                                       id="updateLinksTitle">
                                <label for="floatingModalSignature">Подпись</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" placeholder="Добавьте ссылку" name="updateLinksLink" id="updateLinksLink">
                                <label for="floatingModalLink">Ссылка</label>
                                <div class="invalid-feedback" id="invalidUpdateFeedbackField">
                                    Пожалуйста, заполните поле
                                </div>
                                <div class="invalid-feedback" id="invalidUpdateFeedbackLink">
                                    Поле не является ссылкой
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="submit" name="updateMaterialLink" id="updateMaterialLink" class="btn btn-primary">Редактировать</button>
                            <button type="button" class="btn btn-outline-primary" data-bs-dismiss="modal">Закрыть</button>
                        </div>
                    </div>
                </div>
